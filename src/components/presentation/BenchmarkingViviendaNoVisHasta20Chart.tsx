...
export default function BenchmarkingViviendaNoVisHasta20Chart() {
    ...
    const data = globalData.benchmarkingViviendaNoVisHasta20;
    ...
    const handleUpdate = (index: number, field: string, value: string) => {
        ...
        updateSection('benchmarkingViviendaNoVisHasta20', newData);
    };
        ...
        <div className="w-full h-[580px] p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm shadow-2xl flex flex-col relative">
            ...
            <div className="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p className="text-cyan-500 font-semibold text-lg">Hasta 20 años</p>
                </div>
                <div className="flex gap-4 items-center">
                    <div className="bg-cyan-500/10 px-4 py-2 rounded-xl border border-cyan-500/20 text-center">
                        ...
                        <p className="text-xl font-bold text-cyan-400">11,37% <span className="text-sm font-normal text-slate-400">ea</span></p>
                        <p className="text-xs text-cyan-500/80 font-medium">$1.213</p>
                    </div>
                    <button
                        ...
                        <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                            ...
                            <Bar dataKey="disbursements_num" name="# Desembolsos" fill="#075985" radius={[4, 4, 0, 0]}>
                                {data.map((entry: any, index: number) => (
                                    <Cell key={`cell-${index}`} fill={entry.entity === 'PRESENTE' ? '#06b6d4' : '#075985'} />
                                ))}
                            </Bar>
                            <Bar dataKey="amount" name="$ Desembolsos" fill="#ea580c" radius={[4, 4, 0, 0]}>
                                {data.map((entry: any, index: number) => (
                                    <Cell key={`cell-${index}`} fill={entry.entity === 'PRESENTE' ? '#22d3ee' : '#ea580c'} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                ...
                {isEditing && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="text-xl font-semibold text-white">Editar Datos Vivienda NO VIS (Hasta 20)</h4>
                                        ...
                                        <tr key={index} className="border-b border-slate-800 hover:bg-white/5">
                                            ...
                                            <td>
                                                <input
                                                    ...
                                                    className="bg-transparent border border-slate-700 rounded px-2 py-1 w-24 outline-none text-white focus:border-cyan-500"
                                                />
                                            </td>
                                            <td>
                                                <input
                        ...
                        <div className="mt-6 flex justify-end">
                            <button
                                ...
                                className="px-6 py-2 bg-cyan-600 text-white rounded-lg font-medium"
                            >
                                Guardar y Cerrar
                            </button>
                        </div>
