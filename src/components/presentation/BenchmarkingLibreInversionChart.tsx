...
export default function BenchmarkingLibreInversionChart() {
    ...
    if (!existingMonths.includes('julio')) {
        existingMonths.push('julio');
        existingMonths.sort((a, b) => MONTH_ORDER.indexOf(a) - MONTH_ORDER.indexOf(b));
    }
    ...
    const data = globalData[sectionKey] || globalData[`${prefix}Julio`] || globalData[prefix] || [];
    ...
    const handleAddMonth = (monthKey: string) => {
        const latestMonth = existingMonths[existingMonths.length - 1] || 'julio';
        ...
        const latestData = globalData[latestKey] || data;
    ...
    return (
        <div className="w-full h-[580px] p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm shadow-2xl flex flex-col relative">
            ...
            <div className="flex justify-between items-start mb-4">
                <EditableChartTitle
                    mainTitle="Benchmarking - Libre Inversión"
                    subtitle="LIBRE INVERSIÓN"
                    monthLabel={MONTH_FULL_LABELS[selectedMonth] || selectedMonth}
                    subtitleColor="text-pink-500 font-semibold text-lg"
                />
                        ...
                        <div className="flex bg-slate-800 rounded-lg border border-white/10 overflow-hidden flex-wrap">
                            {existingMonths.map(m => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 text-xs font-bold transition-all ${selectedMonth === m ? 'bg-pink-600 text-white' : 'text-slate-400 hover:text-white'}`}
                                >
                                    {MONTH_LABELS[m] || m}
                                </button>
                            ))}
                ...
                <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                            ...
                            <Bar dataKey="disbursements_num" name="# Desembolsos" fill="#075985" radius={[4, 4, 0, 0]}>
                                {data.map((entry: any, index: number) => (
                                    <Cell key={`cell-${index}`} fill={entry.entity === 'PRESENTE' ? '#ec4899' : '#075985'} />
                                ))}
                            </Bar>
                            <Bar dataKey="amount" name="$ Desembolsos" fill="#db2777" radius={[4, 4, 0, 0]}>
                                {data.map((entry: any, index: number) => (
                                    <Cell key={`cell-${index}`} fill={entry.entity === 'PRESENTE' ? '#f472b6' : '#db2777'} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                ...
                {isEditing && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="text-xl font-semibold text-white">Editar Datos Benchmarking Libre Inversión</h4>
                            ...
                            <table className="w-full text-sm text-left text-slate-400">
                                <thead className="text-xs uppercase bg-slate-800 text-slate-200 sticky top-0">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-pink-400">$ Monto</th>
                                        ...
                                        <tr key={index} className="border-b border-slate-800 hover:bg-white/5">
                                            <td className="px-4 py-2">
                                                <input
                                                    ...
                                                    className="bg-transparent border border-slate-700 rounded px-2 py-1 w-full min-w-[140px] outline-none text-white font-medium focus:border-pink-500"
                                                />
                                            </td>
                                            <td>
                                                <input
                        ...
                        <div className="mt-6 flex justify-end">
                            <button
                                ...
                                className="px-6 py-2 bg-pink-600 text-white rounded-lg font-medium"
                            >
                                Guardar y Cerrar
                            </button>
                        </div>
