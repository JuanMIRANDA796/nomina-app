...
export default function BenchmarkingViviendaNoVisUvrHasta20Chart() {
    ...
    const data = selectedMonth === 'diciembre'
        ? globalData.benchmarkingViviendaNoVisUvrHasta20
        : globalData.benchmarkingViviendaNoVisUvrHasta20Enero;

    const sectionKey = selectedMonth === 'diciembre' ? 'benchmarkingViviendaNoVisUvrHasta20' : 'benchmarkingViviendaNoVisUvrHasta20Enero';
            ...
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="text-2xl font-bold text-white">
                        Benchmarking - Compra de vivienda NO VIS UVR <span className="text-yellow-400">{selectedMonth === 'diciembre' ? 'Diciembre' : 'Enero'}</span>
                    </h3>
                    <p className="text-yellow-500 font-semibold text-lg">Hasta 20 años</p>
                </div>
                <div className="flex gap-3 items-center">
                    <div className="flex bg-slate-800 rounded-lg border border-white/10 overflow-hidden">
                        <button onClick={() => setSelectedMonth('diciembre')} className={`px-3 py-1.5 text-xs font-bold transition-all ${selectedMonth === 'diciembre' ? 'bg-yellow-600 text-white' : 'text-slate-400 hover:text-white'}`}>Dic</button>
                        <button onClick={() => setSelectedMonth('enero')} className={`px-3 py-1.5 text-xs font-bold transition-all ${selectedMonth === 'enero' ? 'bg-yellow-600 text-white' : 'text-slate-400 hover:text-white'}`}>Ene</button>
                    </div>
                    <div className="bg-yellow-500/10 px-4 py-3 rounded-xl border border-yellow-500/20 text-center min-w-[120px]">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Presente</p>
                        <p className="text-xl font-bold text-yellow-400 leading-none">{presenteTpp != null ? `${presenteTpp.toFixed(2).replace('.', ',')}%` : '-'} <span className="text-sm font-normal text-slate-400">ea</span></p>
                        {diff !== null && (
                        ...
                        <p className="text-[10px] text-yellow-500/60 font-medium mt-1">${presenteMonto.toLocaleString()}</p>
                    </div>
                ...
                <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                            ...
                            <Bar dataKey="disbursements_num" name="# Desembolsos" fill="#1e3a8a" radius={[4, 4, 0, 0]}>
                                {data.map((entry: any, index: number) => (
                                    <Cell key={`cell-${index}`} fill={entry.entity === 'PRESENTE' ? '#eab308' : '#1e3a8a'} />
                                ))}
                            </Bar>
                            <Bar dataKey="amount" name="$ Desembolsos" fill="#ea580c" radius={[4, 4, 0, 0]}>
                                {data.map((entry: any, index: number) => (
                                    <Cell key={`cell-${index}`} fill={entry.entity === 'PRESENTE' ? '#fde047' : '#ea580c'} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                ...
                {isEditing && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="text-xl font-semibold text-white">Editar Datos Vivienda NO VIS UVR (Hasta 20)</h4>
                                        ...
                                        <tr key={index} className="border-b border-slate-800 hover:bg-white/5">
                                            ...
                                            <td>
                                                <input
                                                    ...
                                                    className="bg-transparent border border-slate-700 rounded px-2 py-1 w-24 outline-none text-white focus:ring-1 focus:ring-yellow-500"
                                                />
                                            </td>
                                            <td>
                                                <input
                        ...
                        <div className="mt-6 flex justify-end">
                            <button
                                ...
                                className="px-6 py-2 bg-yellow-600 text-white rounded-lg font-medium"
                            >
                                Guardar y Cerrar
                            </button>
                        </div>
