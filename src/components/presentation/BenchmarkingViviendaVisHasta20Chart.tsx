...
export default function BenchmarkingViviendaVisHasta20Chart() {
    ...
    const sectionKey = selectedMonth === 'diciembre' ? 'benchmarkingViviendaVisHasta20' : 'benchmarkingViviendaVisHasta20Enero';
            ...
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="text-2xl font-bold text-white">
                        Benchmarking - Compra de vivienda VIS pesos <span className="text-emerald-400">{selectedMonth === 'diciembre' ? 'Diciembre' : 'Enero'}</span>
                    </h3>
                    <p className="text-emerald-500 font-semibold text-lg">Hasta 20 años</p>
                </div>
                <div className="flex gap-3 items-center">
                    ...
                    <div className="bg-emerald-500/10 px-4 py-3 rounded-xl border border-emerald-500/20 text-center min-w-[120px]">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Presente</p>
                        <p className="text-xl font-bold text-emerald-400 leading-none">{presenteTpp != null ? `${presenteTpp.toFixed(2).replace('.', ',')}%` : '-'} <span className="text-sm font-normal text-slate-400">ea</span></p>
                        {diff !== null && (
                        ...
                        <p className="text-[10px] text-emerald-500/60 font-medium mt-1">${presenteMonto.toLocaleString()}</p>
                    </div>
