...
export default function BenchmarkingViviendaVisUvrSup20Chart() {
    ...
    const data = selectedMonth === 'diciembre'
        ? globalData.benchmarkingViviendaVisUvrSup20
        : globalData.benchmarkingViviendaVisUvrSup20Enero;

    const sectionKey = selectedMonth === 'diciembre' ? 'benchmarkingViviendaVisUvrSup20' : 'benchmarkingViviendaVisUvrSup20Enero';
            ...
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="text-2xl font-bold text-white">
                        Benchmarking - Compra de vivienda VIS UVR <span className="text-lime-400">{selectedMonth === 'diciembre' ? 'Diciembre' : 'Enero'}</span>
                    </h3>
                    <p className="text-lime-500 font-semibold text-lg">Superior a 20 años</p>
                </div>
                <div className="flex gap-3 items-center">
                    <div className="flex bg-slate-800 rounded-lg border border-white/10 overflow-hidden">
                        <button onClick={() => setSelectedMonth('diciembre')} className={`px-3 py-1.5 text-xs font-bold transition-all ${selectedMonth === 'diciembre' ? 'bg-lime-600 text-white' : 'text-slate-400 hover:text-white'}`}>Dic</button>
                        <button onClick={() => setSelectedMonth('enero')} className={`px-3 py-1.5 text-xs font-bold transition-all ${selectedMonth === 'enero' ? 'bg-lime-600 text-white' : 'text-slate-400 hover:text-white'}`}>Ene</button>
                    </div>
                    <div className="bg-lime-500/10 px-4 py-3 rounded-xl border border-lime-500/20 text-center min-w-[120px]">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Presente</p>
                        <p className="text-xl font-bold text-lime-400 leading-none">{presenteTpp != null ? `${presenteTpp.toFixed(2).replace('.', ',')}%` : '-'} <span className="text-sm font-normal text-slate-400">ea</span></p>
                        {diff !== null && (
                        ...
                        <p className="text-[10px] text-lime-500/60 font-medium mt-1">${presenteMonto.toLocaleString()}</p>
                    </div>
                ...
                {isEditing && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="text-xl font-semibold text-white">Editar Datos Vivienda VIS UVR (Sup 20)</h4>
